  ...
  <div class="min-h-screen bg-bg-base font-mono text-text-primary">
    ...
    <header class="border-b border-border bg-bg-surface">
      <div class="mx-auto flex h-14 max-w-7xl items-center justify-between px-4">
        ...
        <nav v-if="session" class="flex items-center gap-4">
          <NuxtLink
            to="/dashboard"
          ...
          <span class="text-sm text-text-muted">{{ session.user.email }}</span>
        </nav>

        <nav v-else-if="!isPending" class="flex items-center gap-4">
          <NuxtLink
            to="/login"
            class="text-sm text-text-secondary hover:text-text-primary"
          >
